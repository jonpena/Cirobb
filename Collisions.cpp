...
* ContactPoint = c1 + n * ((r1 - r2 + |c2 - c1|) * 0.5f) --> The point in the middle. This is a way of doing it.
...
Circle to AABB(Axis Aligned Bounding Box) by moving the OBB to its local space Transforming it in an AABB (Axis Aligned Bounding Box).
...
*************************************************************************************************/
//Collision Detection Circle to Rectangle Oriented
void CircleToOBB(Manifold& m, Shape* c1, Shape*	b1)
{
	...
	real dx = absf(localSpace.x) - b1->width.x * real(0.5);

	if(dx > c1->radius) return;

	real dy = absf(localSpace.y) - b1->width.y * real(0.5);

	if(dy > c1->radius) return;

	Vec2 closest(dx > 0 ? dx : 0, dy > 0 ? dy : 0);

	if(localSpace.x > 0.0f) closest.x *= -1;
	if(localSpace.y > 0.0f) closest.y *= -1; 
	...
	m.postPosition.point2[0] = distance + m.normal * magnitude;
	m.postPosition.orientationOldB = b1->body->orientation;
}
...
{
	m.A = B->body;  m.B = A->body;  CircleToOBB(m, B, A);
}
...
We can also transform the collision detection problem into an AABBToOBB test by simplifying the problem.
**********************************************************************************************/
...
//Collision Detection Rectangle Oriented To Rectangle Oriented
void OBBToOBB(Manifold& m, Shape* b1, Shape* b2)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		if(penetration <= 0)
		{
			...
			//Post-Projection Position
			m.postPosition.point1[numContacts] = b1->body->position + clipPoint - m.A->position;
			m.postPosition.point2[numContacts] = b1->body->position + ref0Point - m.B->position;
			numContacts++;
		}
	}
	//Post-Projection Position
	m.postPosition.orientationOldA = m.A->orientation;
	m.postPosition.orientationOldB = m.B->orientation;
